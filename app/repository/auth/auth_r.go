...
func (repo *AuthRepo) Login(Username string) (model.User, []string, error) {
	var user model.User
	query := `SELECT uuid, name, email, username, password, role_uuid, created_at, updated_at, deleted_at FROM users 
...
func (repo *AuthRepo) Register(request *model.Register) (model.User, []string, error) {
	...
	RETURNING uuid, name, email, username, password, role_uuid, created_at, updated_at, deleted_at`
	var user model.User
